...
int main() {
    vector<string> files = OpenFileDialog();
    
    if (files.empty()) {
        cout << "Nie wybrano plików." << endl;
        return 0;
    }
    int choice;
    cout << "Wybierz operacjê: 1 - Szyfrowanie, 2 - Deszyfrowanie: ";
    cin >> choice;
    string key;
    cout << "Podaj klucz: ";
    cin >> key;
    system("cls");
    key=generate_good_key(key);
    for (const auto& file : files) {
        if (choice == 1) {
        	
            szyfruj(file, key);
            cout << "Zaszyfrowano: " << file << endl;
        } else if (choice == 2) {
            deszyfruj(file, key);
            cout << "Odszyfrowano: " << file << endl;
        }
    }
    Beep(500,200);
    Beep(500,200);
